...
class MovieListServiceSpec: QuickSpec {
    @Injected static var mockManager: MovieListServiceProtocol
    ...
    override class func spec() {
        describe("check movie list service") {
            ...
            context("when successful json data") {
                beforeEach {
                    moviesFromData = Data.jsonDataToObject(Config.MockMovieList.movieListResponseResult)
                    anotherMoviesFromData = Data.jsonDataToObject(Config.MockMovieList.anotherMovieListResponseResult)
    ...
    class func fetchMoviesResult() -> Result<Movies, Swift.Error> {
        var mainResult: Result<Movies, Swift.Error>?
        waitUntil { done in
