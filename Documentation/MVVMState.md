...
protocol MoviesListViewModelProtocol: LifecycleProtocol, ObservableLoadableProtocol
  where GENERIC_RES_TYPE == [MoviesListViewModel.MovieItem], GENERIC_REQ_TYPE == Int {
  func onActive()
  func onBackground()
}
```

This protocol inherits from two other protocols: LifecycleProtocol and ObservableLoadableProtocol. Additionally, it specifies two associated types (GENERIC_RES_TYPE and GENERIC_REQ_TYPE) with specific types: GENERIC_RES_TYPE is an array of MovieItem, and GENERIC_REQ_TYPE is an Int. The protocol also requires conforming types to implement two functions: onActive() and onBackground().
...
Define the typealiases GENERIC_RES_TYPE and GENERIC_REQ_TYPE as specified in the protocol to be used within the class:
